...
const Slider = memo(function Slider(props) {
  const {
    title, currentStartTime,
    currentEndTime, currentStartTimeChange, currentEndTimeChange
  } = props
  ...
  const [startTime, setStartTime] = useState(() => (currentStartTime / 24) * 100)
  ...
  const onStartTouchMove = (e) => {
    ...
    setStartTime(startTime => startTime + (distance / rangeWidth.current) * 100)
  }
  ...
  const onEndTouchMove = (e) => {
    ...
    setEndTime(endTime => endTime + (distance / rangeWidth.current) * 100)
  }
    ...
    <div className='option'>
      ...
      <div className="range-slider">
        <div className="slider">
          ...
          ></div>
          <i className="slider-handle" style={{ left: startPercent + '%' }} ref={startHandle}>
            <span>{startHour}</span>
          </i>
          <i className="slider-handle" style={{ left: endPercent + '%' }} ref={endHandle}>
            <span >{endHour}</span>
          </i>
        </div>
      </div>
